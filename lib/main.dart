...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<List<OptionModel>> getDropdown1Value() async {
    var baseUrl =
        "/*use your 1st dropdown api url*/"; // eg., https://gssskhokhar.com/api/classes/
    var headers = {
    ...
    if (jsonData["message"] == "") {
      for (var data in jsonData["options"]) {
        OptionModel dropDownModelData =
            OptionModel(id: data['id'], val: data['value']);
  ...
  Future<List<OptionModel>> getDropdown2Value(String newvalue) async {
    ...
    var body = <String, String>{"data": dropdownId!};
            ...
            FutureBuilder<List<OptionModel>?>(
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  var data = snapshot.data!;
                  print(
                      "data ${data}  ${dropdownData2.length}  ${dropdownvalue2}");
                  return Container(
                      decoration: BoxDecoration(
                        color: Colors.white,
                        border: Border.all(),
